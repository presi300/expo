...
###Note
By default, the sqlite database file is created on the device itself. On android, it can be accessed by going to `/data/data/{Your_App}/files/SQLite` via adb shell or through an emulator console.
...
The `expo-sqlite` library provides [`AsyncStorage`](#sqliteasyncstorage) as a drop-in replacement for the [`@react-native-async-storage/async-storage`](https://github.com/react-native-async-storage/async-storage) library. This key-value store is backed by SQLite. If your project already uses `expo-sqlite`, you can leverage `AsyncStorage` from `expo-sqlite` without needing to add another dependency.
...
A key benefit of using `AsyncStorage` from `expo-sqlite` is the addition of synchronous APIs for added convenience:
...
If you're currently using `@react-native-async-storage/async-storage` in your project, switching to `expo-sqlite`'s `AsyncStorage` is as simple as changing the import statement:
